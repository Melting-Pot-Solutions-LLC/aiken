<?php
if (isset($_POST['action'])) { // Checking for submit action
	$my_email = 'aikenexpress@yahoo.com'; // Change with your email address
	
	if ($_POST['action'] == 'add') {
		$name	    = trim(strip_tags(addslashes($_POST['name'])));
		$email	    = trim(strip_tags(addslashes($_POST['email'])));
		$phone	    = trim(strip_tags(addslashes($_POST['phone'])));
		$homeaddress= trim(strip_tags(addslashes($_POST['homeaddress'])));
		$city	    = trim(strip_tags(addslashes($_POST['city'])));
		$state	    = trim(strip_tags(addslashes($_POST['state'])));
		$zip	    = trim(strip_tags(addslashes($_POST['zip'])));
		$subject    = trim(strip_tags(addslashes($_POST['subject'])));
		$message    = trim(strip_tags(addslashes($_POST['message'])));
		$pattern    = '/^(?!(?:(?:\\x22?\\x5C[\\x00-\\x7E]\\x22?)|(?:\\x22?[^\\x5C\\x22]\\x22?)){255,})(?!(?:(?:\\x22?\\x5C[\\x00-\\x7E]\\x22?)|(?:\\x22?[^\\x5C\\x22]\\x22?)){65,}@)(?:(?:[\\x21\\x23-\\x27\\x2A\\x2B\\x2D\\x2F-\\x39\\x3D\\x3F\\x5E-\\x7E]+)|(?:\\x22(?:[\\x01-\\x08\\x0B\\x0C\\x0E-\\x1F\\x21\\x23-\\x5B\\x5D-\\x7F]|(?:\\x5C[\\x00-\\x7F]))*\\x22))(?:\\.(?:(?:[\\x21\\x23-\\x27\\x2A\\x2B\\x2D\\x2F-\\x39\\x3D\\x3F\\x5E-\\x7E]+)|(?:\\x22(?:[\\x01-\\x08\\x0B\\x0C\\x0E-\\x1F\\x21\\x23-\\x5B\\x5D-\\x7F]|(?:\\x5C[\\x00-\\x7F]))*\\x22)))*@(?:(?:(?!.*[^.]{64,})(?:(?:(?:xn--)?[a-z0-9]+(?:-+[a-z0-9]+)*\\.){1,126}){1,}(?:(?:[a-z][a-z0-9]*)|(?:(?:xn--)[a-z0-9]+))(?:-+[a-z0-9]+)*)|(?:\\[(?:(?:IPv6:(?:(?:[a-f0-9]{1,4}(?::[a-f0-9]{1,4}){7})|(?:(?!(?:.*[a-f0-9][:\\]]){7,})(?:[a-f0-9]{1,4}(?::[a-f0-9]{1,4}){0,5})?::(?:[a-f0-9]{1,4}(?::[a-f0-9]{1,4}){0,5})?)))|(?:(?:IPv6:(?:(?:[a-f0-9]{1,4}(?::[a-f0-9]{1,4}){5}:)|(?:(?!(?:.*[a-f0-9]:){5,})(?:[a-f0-9]{1,4}(?::[a-f0-9]{1,4}){0,3})?::(?:[a-f0-9]{1,4}(?::[a-f0-9]{1,4}){0,3}:)?)))?(?:(?:25[0-5])|(?:2[0-4][0-9])|(?:1[0-9]{2})|(?:[1-9]?[0-9]))(?:\\.(?:(?:25[0-5])|(?:2[0-4][0-9])|(?:1[0-9]{2})|(?:[1-9]?[0-9]))){3}))\\]))$/iD';
		
		if ($email != '' && $message != '') {
			if (preg_match($pattern, $email)) {
				$headers = 'From: ' . $email . "\r\n";
                $msg = "Name: ".$name."   Email: ".$email."     Phone: ".$phone."      Homeaddress: ".$homeaddress."      City: ".$city."      State: ".$state."      Zip: ".$zip."      Message: ".$message;
				mail($my_email, "Aiken website", $msg, $headers);
				echo 'success|Send a message process completed.';
			} else {
				echo 'error|Please enter a valid email address!';
			}
		} else {
			echo 'error|Please fill all the required fields!';
		}
	}
} else { // Submit through invalid form
	echo 'error|Please submit data through a valid form!';
}
